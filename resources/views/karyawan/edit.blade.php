...
@section('title')
    <h1><i class="bi bi-pencil-square me-2"></i>Edit Karyawan</h1>
@endsection
...
<div class="row">
    <div class="col-lg-8 mx-auto">
        <div class="card border-0 p-4">
            <h5 class="fw-bold text-primary mb-4">Edit Data Karyawan</h5>
        ...
        <div class="mb-3">
            ...
            <input type="text" class="form-control" id="NoTlp" name="NoTlp"
                value="{{ old('NoTlp', $karyawan->NoTlp) }}" maxlength="14" required>
            @error('NoTlp')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
        ...
        <div class="mb-3">
            ...
            <input type="date" class="form-control" id="TglMasuk" name="TglMasuk"
                value="{{ old('TglMasuk', $karyawan->TglMasuk ? $karyawan->TglMasuk->format('Y-m-d') : '') }}" required>
            @error('TglMasuk')
                <div class="text-danger">{{ $message }}</div>
            @enderror
        </div>
